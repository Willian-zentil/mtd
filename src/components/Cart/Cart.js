...
function Cart({ items, onRemoveItem, onClearCart  }) {
    ...
    const handleStartNewOrder = () => {
        onClearCart(); // Limpa o carrinho e as seleções
        ...
        <div className='cart'>
            ...
            {items.length === 0 ? (
                ...
                <div>
                    <div className='content-items'>
                        {items.map((item) => (
                            <div key={item.id} className='cart-item'>
                                ...
                                <div>
                                    <span className='qtd'>{item.quantity}x</span>
                                    <span>@ ${(item.price).toFixed(2)}</span>
                                    <span>$<b>{(item.price * item.quantity).toFixed(2)}</b></span>
                                </div>
                                <span onClick={() => onRemoveItem(item.id)} className='remove-item'>
                                    X
                                </span>
                            </div>
                        ))}
                    ...
                    <input type='button' value='Confirm Order' onClick={handleConfirmOrder} />
                    ...
                    {isPopupOpen && (
                        <div className='popup'>
                            <div className='popup-content'>
                                    ...
                                    {items.map((item) => (
                                        <li key={item.id}>
                                            ...
                                            <div className='content-thumb'>
                                                <span className='name'>{item.name}</span>
                                                <div>
                                                    <span class="qtd">{item.quantity}x</span> <span>@ ${(item.price).toFixed(2)}</span>
                                                    <span className='subtotal-item'><b>${(item.price * item.quantity).toFixed(2)}</b></span>
                                                </div>
                                            </div>
                                ...
                                <input onClick={handleStartNewOrder}  value={'Start New Order'} />
                            </div>
                        </div>
                    )}
                </div>
